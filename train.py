...
def compute_loss(name_scope,out_regress,targets_normalized,aux_cost_weight):
    ...
    total_loss  = mse_autoregressive_steering + aux_cost_weight * (mse_autoregressive)
...
def placeholder_inputs(batch_size):
    ...
    inputs_placeholder = tf.placeholder(tf.float32, shape=(batch_size,
                                                            ...
                                                            WIDTH,
                                                            RGB_CHANNEL))
    # (1+seq_len) x batch_size x OUTPUT_DIM 
    labels_placeholder = tf.placeholder(tf.float32, shape=(batch_size,1+SEQ_LEN,OUTPUT_DIM))
...
with graph.as_default():
    ...
    for gpu_index in range(0, GPU_NUM):
        ...
        with tf.variable_scope('SDC', reuse=bool(gpu_index != 0)):
            with tf.device('/gpu:%d' % gpu_index):
                out_regress = model.inference_att(
                ...
                loss = compute_loss(
                                loss_name_scope,
                                out_regress,
                                targets_normalized[gpu_index * BATCH_SIZE:(gpu_index + 1) * BATCH_SIZE,1:,:],
                                aux_cost_weight)
    ...
    mse_autoregressive_steering = tf.reduce_mean(tf.squared_difference(predictions[:, :, 0], targets_normalized[:, 1:, 0]))
...
def do_epoch(session, sequences, mode):
    ...
    for step in range(total_num_steps):
        ...
        if mode == "train":
            ...
            summary, _, loss = \
                session.run([summaries, optim_op_group, mse_autoregressive_steering],
                           feed_dict = feed_dict)
            ...
            steering_targets = feed_targets[:, 1:, 0].flatten()
